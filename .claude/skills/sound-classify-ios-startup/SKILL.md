...
description: sound_classify_app を iOS 実機で起動するための支援。iOS/Xcode/Flutter 互換性の確認、JITクラッシュ対策、3.41.1 失敗時のフォールバックを含む。iOSシミュレータは非対応で、必ず実機で案内する。
...
3) **FVM前提（必須）**
   ...
   - まずユーザーに `direnv allow` を実行してもらう（対話が必要なため AI からは流さない）。
   - Bash ツールで `flutter --version` を確認し、`3.41.1` でなければ `fvm use 3.41.1` を提案。
   - 未インストールなら `fvm install 3.41.1` を提案。
...
6) **実機起動**
   - `flutter devices` でデバイスIDを取得。
   - `flutter run -d <device-id>` は長時間プロセスなので Bash の `run_in_background` で実行し、ログをモニタする。

## 初回メッセージの締め

- 確認事項と進め方を提示したあと、最後に必ず「上記で進めていいですか？（YES / NO）」を入れる。
- YES を得るまでは破壊的・時間のかかるコマンド（`flutter clean`, `pod install`, `fvm install` 等）は実行しない。
- YES なら以降の手順を Bash ツールで順に実行する。NO なら懸念点の解消を優先する。
...
- `fvm install` が遅い場合は、初回だけ時間がかかることを伝えて待ってもらう（Bash の timeout を延長するか `run_in_background` を使う）。
